...
def get_single_da_label(raw_da_tag):
    ...
    if any(char in ["^"] for char in label):
        ...
        return label  # else the general tag
...
def evaluate(data, window_size, batch_size=100, results_filename="results"):
    ...
    for doc in data:
        ...
        for batch_index in range(0, len(sent), batch_size):
            sent_flat = sent[
                max(0, batch_index - window_size) : min(
                    batch_index + batch_size, seq_len
                )
            ]
    ...
    macro_f1_da = precision_recall_fscore_support(
    ...
    for key, value in dict_report.items():
        ...
        fileOpen.write(
            ...
            + tag_desc_map[tag]
            + ","
            + str(round(value["precision"], 4))
            + ","
            + str(round(value["recall"], 4))
            + ","
            + str(round(value["f1-score"], 4))
            + ","
            + str(value["support"])
            + "\n"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--results_file",
        help="Name of file to write classwise results to",
        default="results",
        type=str,
    )
    parser.add_argument(
        "--ASR", help="Use uncorrected ASR transcripts", default=0, type=int
    )
    parser.add_argument(
        "--model_file",
        help="The trained model weights file to use",
        default="../model/sequential_baseline_roberta_B32_W8_seed1.pt",
        type=str,
    )
